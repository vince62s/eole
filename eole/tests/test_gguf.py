...
class TestGGUFLinear(unittest.TestCase):
    ...
    def test_q4_k_materializes_weight_on_first_call(self):
        """After the first Q4_K forward, weight buffer must be float (materialized)."""
        _require("numpy")
        import torch
        from gguf import GGMLQuantizationType

        layer = self._make_gguf_layer(n_out=8, n_in=256, qtype_enum=GGMLQuantizationType.Q4_K)

        # GGUFLinear is always initialized with a uint8 placeholder weight
        # (register_buffer("weight", torch.zeros(1, dtype=torch.uint8))) and
        # the real quantized weight is loaded as uint8 by load_safe_state_dict.
        # Verify that initial state before any forward call is uint8.
        self.assertEqual(layer.weight.dtype, torch.uint8)

        x = torch.randn(2, 4, 256)
        out1 = layer(x)

        # After first call: weight has been materialized to float
        self.assertTrue(
            layer.weight.is_floating_point(),
            "weight should be float after first Q4_K forward (lazy materialization)",
        )
        self.assertEqual(layer.weight.shape, (8, 256))

    def test_q4_k_second_call_uses_materialized_weight(self):
        """Second Q4_K forward must produce identical output to the first call.

        After materialization the weight is stored as a float buffer.  Both
        calls feed the same input through the same float weight, so the result
        must be bit-for-bit identical (not just numerically close).
        """
        _require("numpy")
        import torch
        from gguf import GGMLQuantizationType
        ...
        # Exact equality: same float weight and same input → identical result
        # (equal_nan=True so NaN values in poorly-quantized test weights also match)
        torch.testing.assert_close(out1, out2, rtol=0, atol=0, equal_nan=True)

    def test_q4_k_output_matches_reference_dequantize(self):
        """Q4_K forward output must match the reference gguf.dequantize result."""
        _require("numpy")
        import torch
        import torch.nn.functional as F
        ...
        # Compute reference BEFORE the forward mutates the weight buffer
        w_np = layer.weight.cpu().numpy()
        dq_np = dequantize(w_np, GGMLQuantizationType.Q4_K)
        w_ref = torch.from_numpy(dq_np).reshape(8, 256).to(torch.float32)

        x = torch.randn(2, 4, 256)
        expected = F.linear(x, w_ref)
        actual = layer(x)

        self.assertEqual(actual.shape, (2, 4, 8))
        torch.testing.assert_close(
            actual.to(torch.float32), expected, rtol=1e-4, atol=1e-4, equal_nan=True
        )
