...
class TestChunkAttnMaskPrefixLM(unittest.TestCase):
    ...
    def _chunk_mask_cpu(self, chunk_size, cache_len, current_step, prefix_len_val,
                        batch=1, sliding_window=0):
        ...
        return TransformerDecoder._chunk_attn_mask(
            mock, chunk_size, current_step, tgt_pad_mask, prefix_len=prefix_len
        )
    ...
    def test_matches_pure_python_reference(self):
        ...
        for chunk_idx in range(4):
            ...
            for q_offset in range(chunk_size):
                ...
                if q_pos >= cache_len:
                    break
                ref = _prefix_chunked_allowed_keys(
                    q_pos, cache_len, current_step, chunk_size, prefix_len_val
                )
                ...
                self.assertEqual(
                    ...
                    msg=(
                        f"chunk={chunk_idx}, q={q_pos}: "
                        f"mask={sorted(actual)} != ref={sorted(ref)}"
                    ),
                )
