...
class MarlinQuantLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Cast scales and bias to match the input dtype without mutating
        # module parameters.  Using replace_parameter() inside forward() is
        # incompatible with torch.compile (fullgraph=True) because it calls
        # untyped_storage().nbytes() and register_parameter(), neither of
        # which are traceable torch ops.  A plain .to() is a proper torch
        # operation that the compiler can optimise away when dtypes already
        # match.
