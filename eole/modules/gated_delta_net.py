...
class GatedDeltaNet(nn.Module):
    ...
    def __init__(self, decoder_config, layer_idx: int):
        ...
        self.in_proj_qkv = skip_init(
            ...
            out_features=self.key_dim * 2 + self.value_dim, bias=False,
        )
        self.in_proj_z = skip_init(
            nn.Linear, in_features=hidden_size,
            out_features=self.value_dim, bias=False,
        )
        self.in_proj_b = skip_init(
            nn.Linear, in_features=hidden_size,
            out_features=self.num_v_heads, bias=False,
        )
    ...
    # ------------------------------------------------------------------
    # Cache helpers (called from TransformerDecoder)
    # ------------------------------------------------------------------

    def init_cache(self, batch_size, dtype, device):
        """Allocate conv and recurrent states for auto-regressive decoding."""
    ...
    def disable_cache(self):
        """Clear inference state (return to training mode)."""
        self.conv_state = None
        self.recurrent_state = None

    def map_state(self, fn):
        """Apply a mapping function to the internal cache (used for beam search)."""
        if self.conv_state is not None:
            self.conv_state = fn(self.conv_state)
        if self.recurrent_state is not None:
            self.recurrent_state = fn(self.recurrent_state)
