...
* eole MarlinQuantLinear – (Marlin tiled layout, no gptqmodel dependency)
...
Marlin (after post_init, for use with moe_wna16_marlin_gemm):
...
def detect_expert_quant_type(experts) -> str:
    ...
    Inspect the first expert gate_up_proj and return one of:
      ...
      'marlin' – eole MarlinQuantLinear / moe_wna16_marlin_gemm
    ...
    # Detect eole's own MarlinQuantLinear (gptqmodel dependency removed)
    if classname == "MarlinQuantLinear" and "eole" in module_path:
        return "marlin"

    # Legacy: also detect gptqmodel MarlinQuantLinear for backward compat
    if "gptqmodel" in module_path and classname == "MarlinQuantLinear":
        return "marlin"
